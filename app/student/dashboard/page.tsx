  ...
  return (
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative z-10">
        ...
        {/* Welcome Section */}
        <div className="mb-10 p-6 sm:p-8 rounded-2xl bg-gradient-to-br from-indigo-950/20 via-slate-900/10 to-background border border-border/50 relative overflow-hidden shadow-sm">
        ...
        <div className="grid sm:grid-cols-3 gap-6 mb-12">
          ...
          <Card className="border-border glass-panel rounded-2xl shadow-sm glow-effect">
        ...
        {activeEnrollments.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-extrabold text-foreground mb-6 flex items-center gap-2">
              <BookOpen className="h-6 w-6 text-primary" /> My Subscribed Courses
            </h2>
              ...
              {activeEnrollments.map((enrollment) => {
                ...
                return (
                  <Link key={enrollment.id} href={`/student/courses/${enrollment.course_id}`}>
                    <Card className="glass-panel border-border hover:shadow-md hover:border-primary/30 transition-all duration-300 h-full flex flex-col justify-between rounded-2xl overflow-hidden cursor-pointer group">
                      <div>
                        {imageUrl && (
                          <div className="w-full h-40 relative overflow-hidden border-b border-border/40">
                            <img
                              src={imageUrl}
                      ...
                      <CardContent className="p-6 pt-0 space-y-5">
                        <div className="border-t border-border/40 pt-4">
                          ...
                          <div className="w-full bg-secondary/40 rounded-full h-2 overflow-hidden">
                            <div className="bg-primary h-2 rounded-full w-0 transition-all duration-500" />
                          </div>
                        </div>
                        <Button className="w-full bg-primary hover:bg-primary/95 text-primary-foreground font-semibold rounded-xl gap-2 h-11">
        ...
        {pendingEnrollments.length > 0 && (
          <div className="mb-12">
            <h2 className="text-2xl font-extrabold text-foreground mb-6 flex items-center gap-2">
              <Clock className="h-6 w-6 text-amber-500" /> Pending Approvals
            </h2>
            <div className="grid md:grid-cols-2 gap-6">
              {pendingEnrollments.map((enrollment) => {
                const { description: cleanDesc, imageUrl } = parseCourseDescription(enrollment.courses?.description)
                return (
                  <Card key={enrollment.id} className="border-amber-500/20 bg-amber-500/5 rounded-2xl overflow-hidden glass-panel flex flex-col justify-between">
                    <div>
                      {imageUrl && (
                        <div className="w-full h-40 relative overflow-hidden border-b border-amber-500/10">
                          <img
                            src={imageUrl}
                            alt={enrollment.courses?.title}
        ...
        {activeEnrollments.length === 0 && pendingEnrollments.length === 0 && (
          <div className="glass-panel text-center py-16 px-6 max-w-lg mx-auto rounded-2xl mt-10">
            <BookOpen className="h-12 w-12 text-muted-foreground/30 mx-auto mb-4" />
            <h3 className="text-xl font-bold text-foreground mb-2">No Active Courses</h3>
            ...
            <Link href="/">
              <Button className="bg-primary hover:bg-primary/95 text-primary-foreground font-semibold rounded-xl px-6 h-11 gap-1.5">
